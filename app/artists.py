...
# Create a table
cursor.execute("CREATE TABLE tracks (id serial PRIMARY KEY, title VARCHAR(150), quantity INTEGER);")

results = sp.search(q='Beastie Boys', limit=50)

for idx, track in enumerate(results['tracks']['items']):
  cursor.execute("INSERT INTO tracks (title, quantity) VALUES (%s, %s);", (track['name'], idx))
